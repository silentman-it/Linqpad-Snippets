<Query Kind="Statements">
  <Connection>
    <ID>a4da0b26-e220-492d-a3ae-4b29358701ef</ID>
    <Persist>true</Persist>
    <Server>172.19.122.63\,3433</Server>
    <SqlSecurity>true</SqlSecurity>
    <UserName>sa</UserName>
    <Password></Password>
    <Database>BIDEVO_DEV</Database>
    <ShowServer>true</ShowServer>
  </Connection>
</Query>

string userName = "federico";
string operatorName = "OEACSMG"; // Iren Mercato

var op = Operators.SingleOrDefault(x => x.EtsoCode == operatorName);
var u = Users.SingleOrDefault(x => x.UserId == userName);

var association = UserOperators.SingleOrDefault(x => x.OperatorId == op.Id && x.UserId == u.Id);

if(association == null)
{
	association = new UserOperators()
	{
		CreationDate = DateTimeOffset.UtcNow,
		UserId = u.Id,
		OperatorId = op.Id
	};
	
	UserOperators.InsertOnSubmit(association.Dump());
	
	SubmitChanges();
}


	
	
